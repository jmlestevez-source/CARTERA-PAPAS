...
# MAPEO DE TICKERS: tu ticker -> ticker en digrin.com
DIGRIN_TICKER_MAP = {
    'IQQM.DE': 'IQQM.DE',
    'TDIV.AS': 'TDIV.AS',
    'EHDV.DE': 'EHDV.DE',
...
def scrape_digrin_dividends(ticker, digrin_ticker):
    ...
    try:
        ...
        if not table:
            table = soup.find('table', {'class': re.compile(r'table.*bordered')})

        if not table:
            dividend_data['source'] = f'digrin.com - tabla no encontrada en {digrin_ticker}'
        ...
        if not rows:
            dividend_data['source'] = f'digrin.com - sin filas en {digrin_ticker}'
            return dividend_data
        ...
        dividend_data['source'] = f'Error conexión: {str(e)[:30]}'
        return dividend_data
    except Exception as e:
        dividend_data['source'] = f'Error: {str(e)[:30]}'
        return dividend_data
...
def get_dividends_from_date(dividend_data, start_date):
    ...
    for ticker, data in dividend_data.items():
        ...
        if history:
            for div in history:
                div_date = div['ex_date'] if isinstance(div['ex_date'], datetime) \
                    else datetime.combine(div['ex_date'], datetime.min.time())
                if div_date > start_dt:
...
st.caption(
    f"📅 Fecha de inicio de inversión: **{start_date.strftime('%d/%m/%Y')}** | "
    ...
    if not full_df.empty:
        ...
        with col_graph:
            ...
            if len(df_analysis) > 0:
                ...
                plot_series_val = pd.concat(
                    [row_init_val, portfolio_series[['Total']]]
                ).sort_index()
                    ...
                    if benchmark_data is not None and len(benchmark_data) > 0:
                        try:
                            ...
                            benchmark_data = benchmark_data[
                                ~benchmark_data.index.duplicated(keep='first')
                            ]
                            ...
                            if benchmark_initial_price > 0:
                                ...
                                if len(benchmark_filtered) > 0:
                                    ...
                                    benchmark_value_series = pd.concat(
                                        [initial_point, benchmark_value_series]
                                    )
                                    benchmark_value_series = benchmark_value_series[
                                        ~benchmark_value_series.index.duplicated(keep='first')
                                    ]
                ...
                fig.update_yaxes(
                    title_text="Valor (€)", gridcolor='#e2e8f0',
                    row=1, col=1, autorange=True, tickformat=","
                )
                fig.update_yaxes(
                    title_text="DD", tickformat=".0%",
        ...
        with col_table:
            ...
            def style_rebal(v):
                ...
                if "COMPRAR" in str(v):
                    return 'color: #1e40af; background-color: #dbeafe; font-weight: bold;'
                return 'color: #166534; background-color: #dcfce7; font-weight: bold;'

            # FIX: usar .map() en lugar de .applymap() (pandas >= 2.1.0)
            st.dataframe(
                df_rb.style.map(style_rebal, subset=['Estado']),
            ...
            <div style="background-color:#ffffff; padding:15px; border-radius:8px;
                        margin-top:15px; border:1px solid #cbd5e1;">
                <span style="color:#475569; font-weight: 600;">Liquidez:</span>
                <span style="color:#0f172a; font-weight:bold; float:right;
                      font-size:1.2rem;">{cash_leftover:.2f} €</span>
            </div>
            """, unsafe_allow_html=True)
...
with tab2:
    ...
    <div style="background-color:#dbeafe; border-left:4px solid #2563eb; padding:12px;
        ...
        <b>ℹ️ Info fiscal:</b> ETFs UCITS (Irlanda/Luxemburgo) sin retención en origen.
        España aplica <b>19%</b>.
    </div>
    """, unsafe_allow_html=True)
    ...
    for ticker, cfg in PORTFOLIO_CONFIG.items():
        ...
        # FIX: incluir mes de inicio (>=) y no depender del año incorrectamente
    ...
    for ticker, cfg in PORTFOLIO_CONFIG.items():
        div_data = DIVIDEND_DATA.get(ticker, {})
        history = div_data.get('history', [])[:6]
    ...
    if history_data:
        ...
        st.dataframe(df_history.head(25), use_container_width=True, hide_index=True)
    else:
    ...
    for i in range(12):
        ...
        with cols[i % 6]:
            ...
            # FIX: lógica corregida para colorear meses
            is_future_payment = mes_num >= today_month if today_year == start_year else True
            is_valid_month = mes_num >= start_month

            if total_mes > 0 and is_future_payment and is_valid_month:
                bg_color = "#dcfce7"
                border_color = "#16a34a"
                icon = "💰"
            elif total_mes > 0 and is_valid_month:
                bg_color = "#fef3c7"
                border_color = "#f59e0b"
                icon = "⏳"
            elif total_mes > 0:
...
with tab3:
    ...
    col4.metric("🗓️ 1er Año Neto", f"€{total_first_year_neto:,.2f}")
    ...
    for ticker, cfg in PORTFOLIO_CONFIG.items():
        ...
        # FIX: usar >= start_month para incluir mes de inicio
        for month in div_months:
            if month >= start_month:
    ...
    for i in range(12):
        ...
        with cols[i % 4]:
            ...
            # FIX: lógica corregida para colorear meses en Tab3
            is_future_payment = mes_num >= today_month if today_year == start_year else True
            is_valid_month = mes_num >= start_month

            if data['bruto'] > 0 and is_future_payment and is_valid_month:
                bg_color = "#dcfce7"
                border_color = "#16a34a"
            elif data['bruto'] > 0 and is_valid_month:
                bg_color = "#fef3c7"
                border_color = "#f59e0b"
            else:
                bg_color = "#f1f5f9"
            ...
            <div style="background-color:{bg_color}; border:2px solid {border_color};
                ...
                <div style="font-weight:bold; color:#1e293b;
                      font-size:0.9rem; margin-bottom:5px;">{meses_nombres[i]}</div>
                <div style="color:#64748b; font-size:0.7rem;">Bruto: €{data['bruto']:.2f}</div>
                <div style="color:#dc2626; font-size:0.7rem;">
                    -€{data['bruto'] * RETENCION_ESPANA:.2f}
                </div>
                <div style="color:#16a34a; font-weight:700;
                      font-size:1.1rem; margin-top:3px;">€{data['neto']:.2f}</div>
    ...
    with col1:
        st.markdown(f"""
        <div style="background-color:#fff; border:2px solid #2563eb;
                    padding:15px; border-radius:10px;">
            <h4 style="color:#1e293b; margin-bottom:10px;">📊 Totales Anuales</h4>
            <table style="width:100%; color:#334155; font-size:0.9rem;">
                <tr>
                    <td>Dividendos Brutos:</td>
                    <td style="text-align:right; font-weight:bold;">€{total_bruto_esp:,.2f}</td>
                </tr>
                <tr>
                    <td>Retención 19%:</td>
                    <td style="text-align:right; color:#dc2626;">-€{total_ret_esp:,.2f}</td>
                </tr>
                <tr style="border-top:1px solid #e2e8f0;">
                    <td style="padding-top:8px;"><b>NETO:</b></td>
                    <td style="text-align:right; font-weight:bold; color:#16a34a;
                          padding-top:8px;">€{total_neto_esp:,.2f}</td>
                </tr>
            </table>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(f"""
        <div style="background-color:#fff; border:2px solid #16a34a;
                    padding:15px; border-radius:10px;">
            <h4 style="color:#1e293b; margin-bottom:10px;">📅 Promedios</h4>
            <table style="width:100%; color:#334155; font-size:0.9rem;">
                <tr>
                    <td>Media Mensual Neto:</td>
                    <td style="text-align:right; font-weight:bold;
                          color:#16a34a;">€{avg_mensual_neto:,.2f}</td>
                </tr>
                <tr>
                    <td>Yield Bruto:</td>
                    <td style="text-align:right;">
                        {(total_bruto_esp / CAPITAL_INVERTIDO) * 100:.2f}%
                    </td>
                </tr>
                <tr>
                    <td>Yield Neto:</td>
                    <td style="text-align:right; font-weight:bold;">{yield_neto:.2f}%</td>
                </tr>
            </table>
        </div>
        """, unsafe_allow_html=True)
